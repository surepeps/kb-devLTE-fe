...
import type { PotentialClientData, NegotiationType, DateTimeObj } from "@/types/negotiation"

export const useNegotiationData = (potentialClientID: string) => {
  const [formStatus, setFormStatus] = useState<"idle" | "success" | "failed" | "pending">("pending")
      ...
      try {
        ...
        if (response.success === true) {
          ...
          if (data.letterOfIntention && data.letterOfIntention !== "") {
            ...
            setNegotiationType(briefType === "Outright Sales" || briefType === "Rent" ? "NORMAL" : "LOI")
          }

          setCreatedAt(data.createdAt || null)

          setDateTimeObj({
            selectedDate: data.inspectionDate ? new Date(data.inspectionDate).toLocaleDateString() : "N/A",
