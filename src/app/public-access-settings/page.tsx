...
export default function PublicAccessSettingsPage() {
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        const token = Cookies.get("token");
        const res = await GET_REQUEST<any>(`${URLS.BASE}/account/public-access/analytics`, token);
        if (res?.success && res.data) {
          const data = res.data as any;
          if (Array.isArray(data.viewsByDay)) setViewsByDay(data.viewsByDay);
          if (data.mostViewedProperty) setMostViewed(data.mostViewedProperty);
        } else {
          // Graceful fallback: try general dashboard for views
  ...
  const SetupHeader = (
    <div className="mb-6">
      <h1 className="text-2xl font-bold text-[#09391C]">Public Access Setup</h1>
      <p className="text-sm text-[#5A5D63] mt-1">Follow the steps to get your public site live.</p>
    </div>
  );

  const ManageHeader = (
    <div className="mb-6">
      <h1 className="text-2xl font-bold text-[#09391C]">Public Access</h1>
      <p className="text-sm text-[#5A5D63] mt-1">Your public site settings and analytics.</p>
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold text-[#09391C]">Your public site is live</h2>
            {previewUrl ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
        <div className="lg:col-span-2 bg-white rounded-lg border border-gray-200 p-6">
          ...
          {analyticsLoading ? (
            <div className="h-40 flex items-center justify-center text-sm text-gray-500">Loading analytics...</div>
          ) : viewsByDay.length > 0 ? (
            <Line
              data={{
