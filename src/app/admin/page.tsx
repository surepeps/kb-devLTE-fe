...
import { useFormik } from 'formik';
import { RegularCard } from '@/components/admincomponents/analysis_overview';
import { calculateAgentCounts, Agent } from '@/utils/agentUtils';
...
export default function AgentManagement() {
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        const response = await GET_REQUEST(URLS.BASE + URLS.getAllAgents, Cookies.get('token'));
        if (response?.success) {
          const agents: Agent[] = response.agents.data;
          setAgentCounts(calculateAgentCounts(agents));
        }
      } catch (error) {
  ...
  return (
    <section className='flex flex-col w-full md:w-[initial]'>
      ...
      <div className='flex md:flex-row flex-col gap-4 mt-6'>
        <RegularCard title='Active Agents' amount={agentCounts.active} />
        <RegularCard title='Total Agents' amount={agentCounts.total} />
        <RegularCard title='Banned Agents' amount={agentCounts.banned} titleColor='#F41515'/>
        <RegularCard title='Flagged Agents' amount={agentCounts.flagged} />
      </div>
