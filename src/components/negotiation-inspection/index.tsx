...
const NegotiationContent: FC<{ potentialClientID: string }> = ({
  ...
  const { formStatus, details, negotiationType, createdAt } =
    useNegotiationDataWithContext(potentialClientID, "buyer");
  ...
  // Enhanced navigation logic for buyer perspective
  useEffect(() => {
    if (details && formStatus === "success") {
      ...
      // Enhanced status checking for buyer experience
      if (
        ...
        // Active negotiations with offers
        goToNextPage("Negotiation");
      } else {
        // Initial state or new negotiations
        goToNextPage("Negotiation");
      }
    }
  ...
  const renderModal = () => {
    ...
    if (formStatus === "success" && details) {
      if (details.negotiationStatus === "completed") {
        return (
          <NegotiationSummary details={details} currentUserType={"buyer"} />
        );
      }
