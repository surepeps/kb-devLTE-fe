...
import JVStep2FeaturesConditions from "@/components/post-property-components/steps/joint-venture/JVStep2FeaturesConditions";
...
const getValidationSchema = (currentStep: number, propertyData: Record<string, unknown>) => {
  switch (currentStep) {
    ...
    case 2:
      return step2ValidationSchema("jv");
    case 3:
      return Yup.object({});
    case 4:
...
const isStepValid = (
  ...
  switch (step) {
    case 0:
      return checkJVStep1RequiredFields(propertyData);
    case 1:
      return checkJVStep2RequiredFields(propertyData);
    case 2:
...
// Helper function to check step 1 required fields for JV
const checkJVStep1RequiredFields = (propertyData: any) => {
...
// Helper function to check step 2 required fields for JV
const checkJVStep2RequiredFields = (propertyData: any) => {
  // isTenanted is required for all property types
  if (!propertyData.isTenanted || propertyData.isTenanted === "") {
    return false;
  }

  // Documents are required for JV
  ...
  const hasJvConditions = propertyData.jvConditions && propertyData.jvConditions.length > 0;
...
const JointVenturePropertyForm: React.FC<JointVenturePropertyFormProps> = ({
  ...
  useEffect(() => {
    updatePropertyData("initializePropertyType", "jv");
  }, [updatePropertyData]);
  ...
  const handleNext = async (
    ...
    switch (currentStep) {
      case 0:
        isCurrentStepValid = checkJVStep1RequiredFields(propertyData);
        break;
      case 1:
        isCurrentStepValid = checkJVStep2RequiredFields(propertyData);
        break;
      case 2:
  ...
  const renderCurrentStep = () => {
    ...
    switch (currentStep) {
      case 0:
        return <JVStep1BasicDetails />;
      case 1:
        return <JVStep2FeaturesConditions />;
      case 2:
        ...
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="text-center mb-6 md:mb-8">
            ...
            <p className="text-[#5A5D63] text-sm md:text-lg max-w-2xl mx-auto px-4">
              {showPropertySummary
                ? "Review your joint venture property listing before submission"
                : showCommissionModal
...
export default JointVenturePropertyForm;
