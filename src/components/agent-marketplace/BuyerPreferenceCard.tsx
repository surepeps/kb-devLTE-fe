      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {preference.urgency && (
              <span
                className={`px-3 py-1 text-sm font-medium rounded-full ${getUrgencyColor(preference.urgency)}`}
              >
                {preference.urgency.charAt(0).toUpperCase() +
                  preference.urgency.slice(1)}{" "}
                Priority
              </span>
            )}
          </div>
            ...
            <div className="bg-gray-50 px-4 py-3 rounded-md">
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium text-[#1E1E1E]">
                  {formatPrice(preference.priceRange.min)}
                </span>
              </div>
              <div className="flex items-center justify-between mt-1">
                <span className="text-[#8D9090]">Maximum:</span>
                <span className="font-medium text-[#1E1E1E]">
                  {formatPrice(preference.priceRange.max)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-[#8D9090] bg-gray-50 px-4 py-2 rounded-md">
              Posted on{" "}
              {new Date(preference.createdAt).toLocaleDateString("en-US", {
                year: "numeric",
                month: "long",
                day: "numeric",
              })}
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <p className="text-sm text-[#8D9090] text-center">
            Ensure your property listing matches this buyer&apos;s preferences
            for better chances of matching.
          </p>
        </div>
      </div>
    </div>
