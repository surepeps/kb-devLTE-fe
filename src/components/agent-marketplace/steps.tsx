...
interface SellProps {
  briefId: string; // Buyer preference ID
  onClose?: () => void;
  buyerPreference?: any;
}

const Sell: React.FC<SellProps> = ({ briefId, onClose, buyerPreference }) => {
  ...
  const [showBuyerPreference, setShowBuyerPreference] = useState(false);
  ...
  // Add login check: if not logged in, redirect to login
  useEffect(() => {
    if (!user) {
  ...
  // Update handleFinalSubmit to use the correct API endpoint and payload
    ...
    try {
      ...
      const payload: any = {
        ...
        landSize: {
          ...
          size: values.landSize ? Number(values.landSize) : '',
        },
        price: values.price ? Number(values.price) : '',
        buildingType: values.typeOfBuilding || '',
        additionalFeatures: {
          noOfBedrooms: values.noOfBedroom ? Number(values.noOfBedroom) : '',
          noOfBathrooms: values.noOfBathroom ? Number(values.noOfBathroom) : '',
          noOfToilets: values.noOfToilet ? Number(values.noOfToilet) : '',
          noOfCarParks: values.noOfCarPark ? Number(values.noOfCarPark) : '',
          additionalFeatures: values.features || [],
        },
        docOnProperty: (values.documents || []).map((doc: string) => ({
          docName: doc,
          isProvided: true,
        })),
        ...
        additionalInfo: values.addtionalInfo || '',
      ...
      await toast.promise(
        fetch(url, {
          ...
          .then(async (res) => {
            ...
            if (response.message === 'Brief created successfully')  {
              ...
              const errorMessage =
                response?.error ||
                response?.message ||
                'Submission failed. Please check your input or try again.';
...
//     setShowSummary(false);
//     setShowFinalSubmit(true);
//     setShowSuccessModal(true); // Show your custom modal
//   } catch (error: any) {
            ...
            <>
              ...
              {currentStep === 0 && !selectedCard && (
                <div>
                  ...
                  {/* View buyer preference link */}
                  {!showBuyerPreference && (
                    <div className='w-full text-center mt-2'>
                      <button
                        className='text-[#1976D2] underline text-base font-medium hover:text-[#0B0D0C] transition'
                        onClick={() => setShowBuyerPreference(true)}
                        type='button'
                      >
                  ...
                  {/* Buyer preference card */}
               {showBuyerPreference && (
  <div className='w-[970px] flex justify-center mt-4'>
    <div className='bg-[#E8F3FE] border border-[#D3E7D9] rounded-lg p-6 w-full max-w-4xl relative'>
      <button
        className='absolute top-3 right-3 text-gray-400 hover:text-gray-600 text-xl'
        onClick={() => setShowBuyerPreference(false)}
        type='button'
      >
        ×
      </button>
      ...
      <div className='grid grid-cols-3 gap-x-8 text-sm'>
        ...
        <div className='space-y-6'>
          <div>
            <div className='text-gray-600 mb-1'>Reference ID</div>
            <div className='text-black font-semibold'>{buyerPreference?.referenceId || '-'}</div>
          </div>
          <div>
            <div className='text-gray-600 mb-1'>Date</div>
            <div className='text-black'>{buyerPreference?.date || '-'}</div>
          </div>
          <div>
            <div className='text-gray-600 mb-1'>Property Type</div>
            <div className='text-black'>{buyerPreference?.propertyType || '-'}</div>
          </div>
          <div>
            <div className='text-gray-600 mb-1'>Location</div>
            <div className='text-black'>{buyerPreference?.location || '-'}</div>
          </div>
        </div>
        ...
        <div className='space-y-6'>
          <div>
            <div className='text-gray-600 mb-1'>Property Prices</div>
            <div className='text-black'>{buyerPreference?.propertyPrice || '-'}</div>
          </div>
          <div>
            <div className='text-gray-600 mb-1'>List type</div>
            <div className='text-black'>{buyerPreference?.listType || '-'}</div>
          </div>
          <div>
            <div className='text-gray-600 mb-1'>Document</div>
            <div className='text-black'>{buyerPreference?.document || '-'}</div>
          </div>
          <div>
            <div className='text-gray-600 mb-1'>Bedroom</div>
            <div className='text-black font-semibold text-lg'>{buyerPreference?.bedroom || '-'}</div>
          </div>
        </div>
        ...
        <div className='space-y-6'>
          <div className='text-right'>
            <div className='text-black font-semibold'>{buyerPreference?.propertyPriceRange || '-'}</div>
          </div>
          <div></div>
          <div></div>
          <div>
            <div className='text-gray-600 mb-1'>Bathroom</div>
            <div className='text-black font-semibold text-lg'>{buyerPreference?.bathroom || '-'}</div>
          </div>
        </div>
      </div>
    </div>
                  ...
                  {currentStep === 1 && (
                    <div className='min-h-[629px] py-[10px] lg:px-[80px] border-[#8D909680] w-full'>
                      ...
                      <div className='w-full min-h-[629px] flex flex-col gap-[30px]'>
                        ...
                        {/* {selectedCard === 'rent' && false && (
                          // Property Condition UI removed
                        )} */}
          ...
          {showSummary && (
            <PropertySummary
              values={{
                ...formik.values,
                // Prevent rendering the whole object directly
                preferenceFeeTransaction: undefined,
              }}
...
interface SuccessBriefModalProps {
  open: boolean;
  onClose: () => void;
  onYes: () => void;
  onNo: () => void;
...
const SuccessBriefModal: React.FC<SuccessBriefModalProps> = ({ open, onClose, onYes, onNo }) => {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-8 flex flex-col items-center relative">
       ...
       <img src="/check.svg" alt="" />
          ...
          <button
            className="w-full bg-[#8DDB90] text-white font-semibold py-3 rounded mb-3 hover:bg-[#6fcf97] transition"
            onClick={onYes}
          >
            Yes I have
          </button>
