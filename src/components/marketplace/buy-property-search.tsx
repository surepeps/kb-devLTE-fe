...
interface BuyPropertySearchProps {
...
const BuyPropertySearch: React.FC<BuyPropertySearchProps> = ({
  propertiesSelected,
  ...
  // Filter properties based on usage options
  useEffect(() => {
    if (usageOptions.length === 0 || usageOptions.includes("All")) {
      setFilteredProperties(properties);
    } else {
      const filtered = properties.filter((property) =>
        usageOptions.some((option) => property.propertyType === option),
      );
      setFilteredProperties(filtered);
    }
    setTotalPages(Math.ceil(filteredProperties.length / itemsPerPage));
    setCurrentPage(1);
  }, [usageOptions, properties]);
              ...
              {getCurrentPageProperties().map((property, index) => (
                <Card
                  ...
                  onCardPageClick={() => {
                    ...
                    router.push(
                      `/property/Buy/${property._id}?selectedBriefs=${selectedBriefsParam}`,
                    );
                  }}
                  onClick={() => {
