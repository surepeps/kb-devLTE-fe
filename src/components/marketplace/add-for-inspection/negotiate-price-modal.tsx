...
const NegiotiatePrice = ({
  ...
  useEffect(() => {
    ...
    if (findSelectedCard) {
      setSelectedProperty((prev) => {
        ...
        if (
          prev.id !== newId ||
          prev.askingPrice !== newAskingPrice ||
          prev.yourPrice !== newYourPrice
        ) {
          return {
            id: newId,
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (typeof selectedProperty.askingPrice === "number" && getID) {
      addNegotiatedPrice(
        getID,
    ...
    setSubmitInspectionPayload((prev) => {
      ...
      const isAnyNegotiated = updatedProperties.some(
        (p) => typeof p.negotiationPrice === "number" && p.negotiationPrice > 0, // Check for positive value
      );
    ...
    (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      ...
      setSubmitInspectionPayload((prev) => {
        ...
        if (selectedProperty.id) {
          // Ensure id exists before attempting to update/add
          ...
          <div className="w-full flex flex-col sm:flex-row gap-4 mt-4">
            ...
            <button
              ...
              disabled={
                Number(yourPriceInput.replace(/,/g, "")) >
                  Number(selectedProperty.askingPrice) ||
                Number(yourPriceInput.replace(/,/g, "")) <= 0
              } // Disable if form is not valid or price is invalid
...
const NegiotiatePriceWithSellerModal: React.FC<NegotiateWithSellerProps> = ({
  ...
  const { addNegotiatedPrice, getNegotiatedPrice } = useMarketplace();
  ...
  useEffect(() => {
    if (availableDates.length > 0) {
      setSubmitInspectionPayload((prev) => {
        ...
        if (
          prev.inspectionDate !== availableDates[0] ||
          prev.inspectionTime !== "9:00 AM"
        ) {
          return {
            ...prev,
  ...
  const formik = useFormik({
    ...
    onSubmit: (values: ContactProps) => {
      ...
      setSubmitInspectionPayload((prev) => {
        ...
        const isAnyNegotiated = updatedProperties.some(
          (p) =>
            typeof p.negotiationPrice === "number" && p.negotiationPrice > 0,
        );
  ...
  useEffect(() => {
    ...
    if (findSelectedCard) {
      const askingPriceVal =
        findSelectedCard?.price ?? findSelectedCard?.rentalPrice;
      setSelectedProperty((prev) => {
        // Only update if values are different to prevent infinite loop
        if (
          prev.id !== findSelectedCard._id ||
          prev.askingPrice !== askingPriceVal
        ) {
          return {
      ...
      >
        <div className="w-full flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-6">
            ...
            <AnimatePresence>
              {isModalOpened && (
                ...
                >
                  ...
                  <div className="p-4 bg-[#8DDB90]/[20%] rounded-md flex flex-col gap-2">
                    {" "}
            ...
            <div className="p-5 bg-[#EEF1F1] rounded-md flex flex-col gap-6">
              {" "}
              {/* Adjusted padding and gap */}
            ...
            {/**buttons */}
            <div className="w-full flex flex-col sm:flex-row gap-4 h-[57px] mt-4">
              {" "}
              {/* Responsive buttons, increased gap */}
              <button
                type="submit"
                disabled={
                  !isFormValid ||
                  Number(formattedYourPrice.replace(/,/g, "")) >
                    Number(selectedProperty.askingPrice) ||
                  Number(formattedYourPrice.replace(/,/g, "")) <= 0
                } // Disable if form is not valid or price is invalid
        ...
        <div className="absolute right-6 bottom-6 z-10 hidden sm:block">
          {" "}
          {/* Hidden on small screens if not needed */}
