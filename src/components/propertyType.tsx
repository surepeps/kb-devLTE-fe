							...
							{["All", "Lease", "Joint Venture", "Outright Sale"].map(
								(item: string, idx: number) => (
									<RadioCheck
										...
										name="Usage Options"
										handleChange={() => {
											const usageOptions = createBrief?.usageOptions?.includes(item)
												? createBrief?.usageOptions?.filter((option) => option !== item)
							...
							createBrief?.fileUrl.map((image) => (
								<ImageContainer
									...
									removeImage={() => {
										...
										setCreateBrief({
											...createBrief,
											fileUrl: createBrief?.fileUrl.filter((img) => img.id !== image.id),
										});
									}}
									image={image.image}
									alt=""
