...
const Index: FC<MainEntryprops> = ({ potentialClientID }) => {
  ...
  useEffect(() => {
    const fetchPotentialClientDetails = async (): Promise<void> => {
      setFormStatus('pending');
      try {
        // Example: adjust URL and response as needed
        // const response = await axios.get(`${URLS.BASE}/inspection/${potentialClientID}`);
const response = await GET_REQUEST(
  `${URLS.BASE + URLS.getMyProperties}/${potentialClientID}`,
  Cookies.get('token')
);
        console.log("response", response);
        if (response.status === 200) {
          setFormStatus('success');
          // Example: adjust these lines based on your backend response
          setNegotiationType(response.data.type); // 'LOL' or 'NORMAL'
          setDetails(response.data.details);
          // Optionally set contentTracker and isNegotiated from response
        }
      } catch (error) {
        setFormStatus('failed');
        console.log(error); 
        ...
        <div className='container flex items-center justify-center'>
          <AnimatePresence>
            {formStatus &&
              renderModal({
                props: {
                  firstName: details?.firstName ?? 'Wale',
                  lastName: details?.lastName ?? 'Abba',
                  contentTracker: contentTracker,
                  setContentTracker: setContentTracker,
                  renderDynamicContent: () => renderContentDynamically(),
                  setIsNegotiated: setIsNegotiated,
...
export default Index;
// ...rest of your code (ConfirmInspectionDate, Price, ShowDateTimeSelected, etc.)...