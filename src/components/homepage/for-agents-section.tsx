        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {loadingPlans
              ...
              : (plans || []).map((plan: any) => (
                  <div key={plan._id} className={`bg-white rounded-2xl p-8 border-2 ${plan.isTrial || (plan.price === 0) ? 'border-gray-200' : 'border-[#8DDB90]'} relative overflow-hidden`}>
                    <div className="text-center mb-6">
                      <h4 className="text-2xl font-bold text-[#09391C] mb-2">{plan.name}</h4>
                      <div className="text-3xl font-bold text-[#8DDB90] mb-2">₦{Number(plan.price || plan.basePrice || 0).toLocaleString()}</div>
                      <p className="text-gray-600 text-sm">
                        {plan.currency || 'NGN'} • {Math.max(1, Math.round((plan.durationInDays || (plan.raw?.durationInDays) || 30) / 30))} month{Math.max(1, Math.round((plan.durationInDays || (plan.raw?.durationInDays) || 30) / 30)) > 1 ? 's' : ''}
                      </p>
                    </div>
                    <ul className="space-y-3 mb-6">
                    ...
                    <div className="mb-4">
                      {(plan.discountedPlans || plan.raw?.discountedPlans || []).map((dp: any, i: number) => (
                        <div key={i} className="flex items-center justify-between text-sm text-gray-700 mb-2">
                          <span>{dp.name}</span>
                          <span className="font-medium">₦{Number(dp.price).toLocaleString()}</span>
                        </div>
                      ))}
                    </div>
