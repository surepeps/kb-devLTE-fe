...
export default function AdminNavbar({ isOpen, setIsOpen }: { isOpen: boolean; setIsOpen: (open: boolean) => void }) {
  ...
  return (
    <Fragment>
      ...
      <div
        className={`fixed z-50 h-full w-[270px] bg-white shadow-lg transition-transform duration-300 ${
            ...
            return (
              <Link
                ...
                className={`flex items-center p-4 rounded-md transition-all duration-200 text-base font-medium ${
                  isActive ? "text-[#8DDB90]" : "text-[#515B6F] hover:bg-gray-200"
                }`}
                onClick={() => setIsOpen(false)}
              >
