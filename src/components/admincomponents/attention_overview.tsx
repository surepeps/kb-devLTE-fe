...
>>>>>>> main
	...
	const renderDynamicComponent = () => {
		switch (active) {
			...
			// 	return (
			// 		<OverdueBriefs
			// 			awaitingApprovalCount={SetAwaitingApprovalCount}
			// 			data={incomingBriefsData}
			// 		/>
			// 	);
				...
				<div className="flex text-lg w-fit gap-4 md:gap-8 mt-6">
					{texts.map((item, index) => (
						<TextNotification
							...
							active={active}
							count={item.text === "Incoming Inspections" ? pendingCount : item.count} // Update count dynamically
						/>
					))}
				</div>
